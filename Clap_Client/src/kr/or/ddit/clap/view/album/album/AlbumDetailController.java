...
//	@FXML Label label_singNo;
	...
	public void initialize(URL arg0, ResourceBundle arg1) {
		...
		label_albumName1.setText(aVO.getSing_name());
		label_albumName2.setText(aVO.getSing_name());
	...
	@FXML public void updateSinger() {
		try {
			//바뀔 화면(FXML)을 가져옴
			UpdateSingerController.albumNo = albumNo;//가수번호를 변수로 넘겨줌
...
/*	@FXML public void deleteSinger() {
		...
		try {
			...
			int cnt =ias.deleteSinger(albumNo);
	...
	public int alertConfrimDelete() {
		Alert alertConfirm = new Alert(AlertType.CONFIRMATION);
	      ...
	      alertConfirm.setContentText("정말로 삭제하시겠습니까?(해당 가수의 앨범 및 앨범이 모두 삭제됩니다)");
